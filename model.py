...
class Generator(nn.Module):
    ...
    def fit_input_shape(self, img, input_size, input_channel):
        img = F.interpolate(img, size=[input_size, input_size])
        img = img.repeat(1, input_channel, 1, 1)
        return img
...
class Discriminator(nn.Module):
    def __init__(self, size, args, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        ...
        # self.final_linear_lr = nn.Sequential(
        #     EqualLinear(channels[4] * 2 * 2, channels[4], activation='fused_lrelu'),
        #     EqualLinear(channels[4], 1),
        # )

        # Local Region
        convs_l = [ConvLayer(3, channels[size], 1)]
        log_size = int(math.log(size, 2))
        in_channel = channels[size]

        for i in range(log_size, 3, -1):
            out_channel = channels[2 ** (i - 1)]
            convs_l.append(ResBlock(in_channel, out_channel, blur_kernel))
            in_channel = out_channel

        self.convs_l = nn.Sequential(*convs_l)
        self.final_conv_l = ConvLayer(in_channel + 1, channels[4], 3)
        self.final_linear_l = nn.Sequential(
            EqualLinear(channels[4] * 4 * 4, channels[4], activation='fused_lrelu'),
            EqualLinear(channels[4], 1),
        )

        # self.convs_l_lr = nn.Sequential(*convs_l)
        # self.final_conv_l_lr = ConvLayer(in_channel + 1, channels[4], 3)
        # self.final_linear_l_lr = nn.Sequential(
        #     EqualLinear(channels[4] * 2 * 2, channels[4], activation='fused_lrelu'),
        #     EqualLinear(channels[4], 1),
        # )
    ...
    def down_sample(self, input):
        ...
        # _, _, w, h =input.shape
        # re_input = F.interpolate(input, size=[int(w/2), int(h/2)])
        re_input = F.avg_pool2d(input, kernel_size=3,
    ...
    def forward(self, input):
        ...
        # out_lr = out_lr.view(batch, -1)
        # out_lr = self.final_linear_lr(out_lr)

        # local face region
        face_ROI = self.slice(input)
        out_l = self.convs_l(face_ROI)
        out_l, batch = self.std_calculate(out_l)
        out_l = self.final_conv_l(out_l)
        ...
        # out_l_lr = self.final_conv_l_lr(out_l_lr)
        # out_l_lr = out_l_lr.view(batch, -1)
        # out_l_lr = self.final_linear_l_lr(out_l_lr)

        # combined and divided by 'None'
        # return (out + out_lr)
        return (out + out_l) / 2 # + out_lr + out_l_lr) / 4
