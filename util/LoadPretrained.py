...
def Load_Test_Pretrained(Model, args):

    if args.test_model_select =='VGGFace2':
        print("Loading {} pretrained model".format(args.test_model_select))

    elif args.test_model_select == 'Light_CNN_9':
        print("Loading {} pretrained model".format(args.test_model_select))
        # Model = WrappedModel(Model)
        checkpoint = torch.load('./Pretrained/LightCNN/LightCNN_9Layers_checkpoint.pth.tar')
        Model.load_state_dict(checkpoint['state_dict'])

    elif args.test_model_select == 'Light_CNN_29':
        print("Loading {} pretrained model".format(args.test_model_select))
        Model = WrappedModel(Model)
        checkpoint = torch.load('./Pretrained/LightCNN/LightCNN_29Layers_checkpoint.pth.tar')
        Model.load_state_dict(checkpoint['state_dict'])

    elif args.test_model_select == 'Light_CNN_29_v2':
        print("Loading {} pretrained model".format(args.test_model_select))
        Model = WrappedModel(Model)
        checkpoint = torch.load('./Pretrained/LightCNN/LightCNN_29Layers_V2_checkpoint.pth.tar')
        Model.load_state_dict(checkpoint['state_dict'])

    elif args.test_model_select == 'IR-50':
        print("Loading {} pretrained model".format(args.test_model_select))
        checkpoint = torch.load('./Pretrained/ms1m_ir50/backbone_ir50_ms1m_epoch63.pth')
        Model.load_state_dict(checkpoint)
    ...
    print("Loading {} successfully!".format(args.test_model_select))
